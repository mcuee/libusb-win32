...
const char winusb_strings_header[] =
"\n"
";--------------------------------------------------------------------------\n"
"; Strings\n"
";--------------------------------------------------------------------------\n"
    ...
    case WM_COMMAND:
      switch(LOWORD(w_param))
        ...
        case ID_BUTTON_NEXT:
          ...
          if(!device->reg.vid)
            {
              device->reg.vid = 0x12AB;
              device->reg.pid = 0x12AB;
            }

          if(!device->reg.manufacturer[0])
            strcpy(device->reg.manufacturer, "Insert manufacturer name");
          if(!device->reg.name[0])
            strcpy(device->reg.name,  "Insert device description");
      ...
      switch(LOWORD(w_param))
        ...
        case ID_BUTTON_NEXT:
          ...
          GetWindowText(GetDlgItem(dialog, ID_TEXT_MANUFACTURER), 
                        device->reg.manufacturer, sizeof(tmp));
          GetWindowText(GetDlgItem(dialog, ID_TEXT_DEV_NAME),
                        device->reg.name, sizeof(tmp));

          GetWindowText(GetDlgItem(dialog, ID_TEXT_VID), tmp, sizeof(tmp));
          sscanf(tmp, "0x%04x", &device->reg.vid);

          GetWindowText(GetDlgItem(dialog, ID_TEXT_PID), tmp, sizeof(tmp));
          sscanf(tmp, "0x%04x", &device->reg.pid);
          
          if(SendMessage(GetDlgItem(dialog, ID_CHECK_WINUSB), 
                         BM_GETCHECK, 0, 0))
            device->use_winusb = TRUE;
          else
            device->use_winusb = FALSE;
...
static void device_list_refresh(HWND list)
{
  ...
  while(usb_registry_get_device(&reg_dev, dev_index++, TRUE)) {
    if(reg_dev.vid && !strstr(reg_dev.driver, "usbhub")) {
      device = malloc(sizeof(device_context_t));
      memset(device, 0, sizeof(*device));
      memcpy(&device->reg, &reg_dev, sizeof(reg_dev));
...
static void device_list_add(HWND list, device_context_t *device)
{
  ...
  sprintf(tmp, "0x%04X", device->reg.vid);
  ListView_SetItemText(list, 0, 0, tmp);
  sprintf(tmp, "0x%04X", device->reg.pid);
  ListView_SetItemText(list, 0, 1, tmp);
  ListView_SetItemText(list, 0, 2, device->reg.name);
}
  ...
  if(GetSaveFileName(&open_file))
    {
      ...
      if(file)
        {
          fprintf(file, "%s", device->use_winusb ? 
                  winusb_inf_header : inf_header);

          fprintf(file, "CatalogFile = %s\n", cat_name);
          fprintf(file, "CatalogFile.NT = %s\n", cat_name);
          fprintf(file, "CatalogFile.NTAMD64 = %s\n\n", cat_name_x64);
          
          fprintf(file, "%s", device->use_winusb ? winusb_inf_body : inf_body);

          fprintf(file, "[Devices]\n");
          fprintf(file, "\"%s\"=LIBUSB_DEV, USB\\VID_%04x&PID_%04x\n\n", 
                  device->reg.name, device->reg.vid, device->reg.pid);
          ...
          fprintf(file, "\"%s\"=LIBUSB_DEV, USB\\VID_%04x&PID_%04x\n\n",
                  device->reg.name, device->reg.vid, device->reg.pid);  
          fprintf(file, device->use_winusb ? winusb_strings_header
                  : strings_header);
