			...
			int endpoint, int timeout)
{
  ...
  KdPrint(("LIBUSB_FILTER - abort_endpoint(): endpoint %02\n"));

  if(!device_extension->current_configuration)
    {
      KdPrint(("LIBUSB_FILTER - abort_endpoint(): invalid configuration 0")); 
  ...
  if(!get_pipe_handle(device_extension, endpoint, 
    ...
    {
      KdPrint(("LIBUSB_FILTER - abort_endpoint(): getting endpoint pipe "
	       "failed\n"));
      return STATUS_UNSUCCESSFUL;
    }
  ...
  if(!NT_SUCCESS(m_status) || !USBD_SUCCESS(urb.UrbHeader.Status))
    {
      KdPrint(("LIBUSB_FILTER - abort_endpoint(): request failed\n"));
